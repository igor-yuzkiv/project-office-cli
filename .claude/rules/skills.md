...
about a skill change. Their place in the phase-3 review is taken by an independent review of
the skill through the `prompt-engineer` skill.

## Verification stage

The verification for a skill change is an independent `prompt-engineer` review, run per the
phase-3 review gate in `workflow.md` — only after the user approves it, in a separate lane
from the authoring context, never self-approved.
...
This is the skills equivalent of the code Validation gate in `workflow.md`; the gating rules
